...
(python /home/mjkim/mjkim_universal_filter/01_universe_filter/00_varscan_somaticfilter_mj.py /home/mjkim/exercise_221107/06_VarScan2/$1.varscan.$2.vcf) &>> $log || { c=$?;echo "Error";exit $c; }
echo done >> $log
#echo unzip vcf.gz file >>$log
#(gzip -dc /home/mjkim/exercise_221107/03_strelka2/$1/results/variants/somatic."$temp_mttype"s.vcf.gz > /home/mjkim/temp/somatic."$temp_mttype"s.vcf) &>> $log || { c=$?;echo "Error";exit $c; }
#echo "done" >>$log
#gz 압축해제 안하면 byte형식 때문에 에러났음
echo "start: union of pass call in varscan2 somatic & strelka2" >>$log
(python /home/mjkim/mjkim_universal_filter/01_universe_filter/00_vcf_combination_by_Youk_$2_mj.py $1 $species 2 /home/mjkim/exercise_221107/03_strelka2/$1/results/variants/somatic."$temp_mttype"s.vcf.gz /home/mjkim/exercise_221107/06_VarScan2/$1.varscan.$2.somatic.vcf) &>> $log || { c=$?;echo "Error";exit $c; }
echo "done" >>$log
#rm /home/mjkim/temp/somatic."$temp_mttype"s.vcf
echo "initial annotation" >> $log
(sh /home/mjkim/mjkim_universal_filter/01_universe_filter/sypark_PointMt_annot_filter/PointMt_annot1111.sh $1_$2_union_2.vcf $4 $5 /home/mjkim/mjkim_universal_filter/01_universe_filter/sypark_PointMt_annot_filter/src $3) &>> $log || { c=$?;echo "Error";exit $c; }
echo "done" >>$log
echo "panel of normal annotation" >> $log
(python /home/mjkim/mjkim_universal_filter/01_universe_filter/02_AddNpanelToVCF_"$2".py $1_$2_union_2.readinfo.readc.rasmy.vcf $6 PanelofNormal $species) &>> $log || { c=$?;echo "Error";exit $c; }
echo "done" >>$log
echo "filter1 using sample and germline information"  >>$log
(python /home/mjkim/mjkim_universal_filter/01_universe_filter/03_$2_filter1.py $1_$2_union_2.readinfo.readc.rasmy_PanelofNormal.vcf) &>> $log || { c=$?;echo "Error";exit $c; } #b6
